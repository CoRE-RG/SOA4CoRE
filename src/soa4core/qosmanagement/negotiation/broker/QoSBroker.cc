        ...
        if (this->isResponsibleFor(msg->getReceiver(), msg->getSender())) {
            ...
            case QoSNegotiationMsgType::QoS_Request:
                handled = handleRequest(
                        dynamic_cast<SOA4CoRE::QoSNegotiationRequest*>(msg));
                break;
            case QoSNegotiationMsgType::QoS_Response:
                handled = handleResponse(
                        dynamic_cast<SOA4CoRE::QoSNegotiationResponse*>(msg));
                break;
            case QoSNegotiationMsgType::QoS_Establish:
                handled = handleEstablish(
                        dynamic_cast<SOA4CoRE::QoSNegotiationEstablish*>(msg));
                break;
            case QoSNegotiationMsgType::QoS_Finalise:
                handled = handleFinalise(
                        dynamic_cast<SOA4CoRE::QoSNegotiationFinalise*>(msg));
                break;
            default:{
...
//            cout << "QoSBroker: full cycle successfull" << endl;
            ...
            if(finalise->getFinalStatus() == QoSNegotiationStatus::Success){
                ...
                ConnectionSpecificInformation* info = dynamic_cast<SOA4CoRE::ConnectionSpecificInformation*>(finalise->decapsulate());
                if(info){
...
void QoSBroker::fillEnvelope(SOA4CoRE::Envelope* envelope) {
    // set receiver
